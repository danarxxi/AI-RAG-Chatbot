...
import { chatAPI, authAPI, sessionAPI, historyAPI } from '../services/api';
...
function Chat() {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      ...
      try {
        const historyData = await chatAPI.getHistory(sessionId);
  ...
  const handleNewChat = async () => {
    ...
    try {
      ...
      // Clear only HR service history (keeps same session, preserves other services)
      await sessionAPI.clearServiceHistory('hr');
  ...
  const handleSelectSession = async (sessionId) => {
    ...
    try {
      const data = await historyAPI.getSessionMessages(sessionId, 'hr');
  ...
  const handleQuickPrompt = (promptText) => {
    ...
    setTimeout(() => {
      ...
      chatAPI.sendMessage(promptText, sessionId)
        .then(response => {
          setMessages(prev => {
            const updated = [...prev];
            const lastIndex = updated.length - 1;
    ...
    <div className="chat-container">
      <div className="chat-header">
        <div className="header-left">
          ...
          <div className="header-titles">
            <h1>RAG CHATBOT</h1>
            <p className="header-subtitle">신입사원을 위한 <span className="subtitle-break">HR 가이드 챗봇</span></p>
          </div>
        </div>
      ...
      <div className="chat-input-container">
        <div className="guide-info-bar">
          ...
          <span className="guide-current">현재 가이드: HR 어시스턴트</span>
        </div>
        {viewingSession ? (
