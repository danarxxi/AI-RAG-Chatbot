...
class RAGService:
    """RAG service using Amazon Bedrock Knowledge Base and Nova model."""
    ...
    def __init__(self):
        ...
        if settings.aws_access_key_id and settings.aws_secret_access_key:
            ...
            logger.warning("AWS credentials not configured - RAG service will use mock responses")
    ...
    def retrieve_context(self, query: str, max_results: int = 5) -> List[Dict[str, Any]]:
        ...
        """
        if not self.bedrock_agent_runtime or not settings.bedrock_knowledge_base_id:
            logger.warning("Bedrock Knowledge Base not configured, returning empty context")
            return []
    ...
    def build_prompt(
        ...
        """
        # System instruction (sent via system parameter, not as user content)
        system_prompt = """\
        You are an AI assistant providing guidance on internal HR system usage for new employees. Your role is to help new hires find accurate information from HR documentation while maintaining a friendly and trustworthy attitude.
        ...
        **Never provide unverified or uncertain information.**
        ...
        인사팀에 직접 문의해 주시기 바랍니다.

        Never attempt to answer questions based on general knowledge or assumptions when the information is not explicitly present in the retrieved HR documents.
        ...
        **Instead:**
        - ✅ Naturally refer to information as coming from "HR 관련 문서" or "인사 관련 문서"
        - ✅ Clearly state when you don't know: "알 수 없습니다" / "확실하지 않습니다"
        - ✅ Provide clear, direct answers **only when the information is explicitly found** in the retrieved HR documentation
        ...
        - **CRITICAL - Tone Override**: The HR documents are written in formal, legal language (제8조, 직원은..., 하여야 합니다). You MUST NOT copy this formal bureaucratic tone. Instead:
            - Translate formal regulations into warm, conversational Korean
            - Use friendly endings: ~하시면 돼요, ~해 주세요, ~하실 수 있어요
            - Avoid stiff endings: ~하여야 합니다, ~할 것, ~하는 바입니다
            - Speak like a helpful HR colleague, NOT like a legal document
            - Make information accessible and easy to understand

        ## Formatting Rules

        - When presenting structured or comparative data (e.g. leave days, benefits, schedules), use **Markdown table format**:
            ```
            | 구분 | 대상 | 일수 | 비고 |
            |------|------|------|------|
            | 결혼 | 본인 | 6일 | 청첩장 |
        ...
        user_parts.append("Here are the relevant HR documents:\n")
    ...
    def generate_response(self, system: str, user_message: str) -> str:
        ...
        if not self.bedrock_runtime:
            ...
            return "I'm a mock response. Please configure AWS Bedrock to get real answers from the HR manual."
    ...
    def get_rag_response(
        ...
        if not context_documents:
            ...
            return {
                'response': '현재 제공된 자료에서는 해당 정보를 찾을 수 없습니다. 인사팀에 직접 문의해 주시기 바랍니다.',
                'sources': []
            }
